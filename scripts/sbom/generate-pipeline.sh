#!/bin/bash
# Generate SBOM for Data Pipeline (Python/Airflow)
...
echo "=== Generating Data Pipeline SBOM ==="
...
if [ ! -f "requirements.txt" ]; then
    echo "Warning: requirements.txt not found in $PIPELINE_DIR"
    echo "Skipping Data Pipeline SBOM generation"
    exit 0
fi
...
$PYTHON -m cyclonedx_py requirements \
    requirements.txt \
    -o "$OUTPUT_DIR/sbom-datapipeline.json" \
    --of JSON \
    --sv 1.5 \
    --mc-type application

# Validate output
if [ -f "$OUTPUT_DIR/sbom-datapipeline.json" ]; then
    COMPONENT_COUNT=$(cat "$OUTPUT_DIR/sbom-datapipeline.json" | grep -c '"name"' || echo "0")
    echo "Data Pipeline SBOM generated successfully!"
    echo "Output: $OUTPUT_DIR/sbom-datapipeline.json"
    echo "Components: ~$COMPONENT_COUNT"
else
    echo "Error: Failed to generate Data Pipeline SBOM"
    exit 1
fi
