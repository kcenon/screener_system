 ...
 * @internal
 */
const processQueue = (error: Error | null, _token: string | null = null) => {
...
// Response interceptor for handling errors and token refresh via HttpOnly cookies
api.interceptors.response.use(
  ...
  async (error: AxiosError) => {
    ...
    // Don't retry for auth endpoints — redirect to login
    ...
    try {
      // Refresh tokens via cookie — no body needed, refresh_token cookie is sent automatically
      await axios.post(`${API_BASE_URL}/auth/refresh`, null, {
        withCredentials: true,
      })

      // Process queued requests (new access_token cookie is now set by backend)
